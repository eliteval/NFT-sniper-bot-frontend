...
const UniSwapOneToken = (props) => {
  ...
  const addHandler = async () => {
    try {
      const response = await ApiCall(
        apiConfig.pan_addBot.url,
        apiConfig.pan_addBot.method,
  ...
  const deletePlan = async (data) => {
    try {
      const response = await ApiCall(
        apiConfig.pan_delBot.url,
        apiConfig.pan_delBot.method,
        props.credential.loginToken,
        data
      );
  ...
  const sell = async (data) => {
    try {
      closeDetailModal();
      const response = await ApiCall(
        apiConfig.pan_letSell.url,
        apiConfig.pan_letSell.method,
        props.credential.loginToken,
        data
      );
  ...
  const del = async (data) => {
    try {
      const response = await ApiCall(
        apiConfig.pan_letDel.url,
        apiConfig.pan_letDel.method,
        props.credential.loginToken,
        data
      );
      if (response.data.data) setLogData(response.data.data);
  ...
  useEffect(() => {
    ...
    async function readPlan() {
      try {
        const response = await ApiCall(
          apiConfig.pan_readPlan.url,
          apiConfig.pan_readPlan.method,
          props.credential.loginToken,
          {}
        );
        if (response.data.data) setPlan(response.data.data);
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on("connect", () => {
        ...
        console.log("connect");
        socket.on("pancake:one:newPlan", (data) => {
