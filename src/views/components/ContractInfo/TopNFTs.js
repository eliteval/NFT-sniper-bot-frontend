    ...
    (async () => {
      try {
        ...
        const response = await ApiCall(
          apiConfig.getTopNFTs.url,
          apiConfig.getTopNFTs.method,
            ...
            <Table responsive>
              <thead className="text-primary">
                <tr>
                  ...
                  <th className="text-left"></th>
                  <th className="text-left">TokenID</th>
                  <th className="text-left">Amount</th>
                  <th className="text-left">From</th>
                  <th className="text-left">To</th>
                  <th className="text-left">Transaction</th>
                  <th className="text-left">Platfrom</th>
                  ...
                  return (
                    <tr key={key}>
                      ...
                      <td className="text-left">
                        <ImageItem
                          src={[
                            `https://img.nftnerds.ai/${address}_${item.tokenID}_96x96`,
                            defaultImage,
                            altimage,
                          ]}
                          size={70}
                      ...
                      <td className="text-left">{item.tokenID}</td>
                      <td className="text-left">
                        {item.originAmount} {item.currency}
                      </td>
                      <td className="text-left">
                        {shortenWallet(item.from)}
                        <a
                      ...
                      <td className="text-left">
                        {shortenWallet(item.txHash)}
                        <a
                          href={`${explorerURL}/address/${item.txHash}`}
                          target={"_blank"}
                        >
                          &nbsp; <i className="tim-icons icon-link-72" />
                        </a>
                      </td>
                      <td className="text-left">{item.platform}</td>
                      {/* <td className="text-left">
...
export default connect(mapStateToProps)(TopNFTs);
